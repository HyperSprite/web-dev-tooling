...
Installed with ```-g```, may also need ```sudo```.
 ...
 - [nodemon]() - Replaces vanilla node, watches js files and restarts on changes, awesome for development.
 - [pm2]() - PM2 is a node processes manager. Automatic clustering, restarts on crash, startup order, logging, there is a lot to like. 
